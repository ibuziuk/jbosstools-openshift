...
import org.jboss.tools.openshift.core.debug.TrackerContributionEvaluation;
	...
	private void startDebugging(IServer server, IDeploymentConfig dc, DebuggingContext debugContext,
		...
		IDebugListener listener = new IDebugListener() {
			...
			public void onDebugChange(DebuggingContext debuggingContext, IProgressMonitor monitor)
				...
				}/* else if (NodeDebuggerUtil.isChromiumAvailable() && NodeDebuggerUtil.isNodeJsProject(server)) {
					NodeRemoteDebugger.attach(server, localPort);
				}*/ else if (isNodeJsProject(server)) {
					TrackerContributionEvaluation.startDebugSessions(server, localPort);
				}
			}
